...
class YouTubeNotifier extends EventEmitter {
  ...
  _onPostRequest(req, res) {
    console.log('new notification from Youtube!');
    ...
    // Deleted Video
    if (req.body.feed['at:deleted-entry']) {
      let data = {
        video: { id: req.body.feed['at:deleted-entry'][0].$.ref.split(':')[2] }
      };
      console.log('YTN- video deleted', data.video.id);
      this.emit('notified-delete', data);
    ...
    if (this.secret) {
      ...
      if (hmac.digest('hex').toLowerCase() !== signature) {
        console.log('YTN- secret did not match');
        return res.sendStatus(200);
      }
    }
    ...
    if (this._recieved.includes(vidId)) {
      ...
      console.log('YTN- already received');
      return res.sendStatus(200);
    }
